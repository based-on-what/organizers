...
- **moviepy 1.0.3+** — lectura de duracion de video
...
Cuenta las paginas de documentos en el directorio actual (no recursivo).
...
Calcula la duracion total de video para cada subcarpeta del directorio actual, tratando cada subcarpeta como una serie de TV diferente.
...
Convierte todos los archivos `.doc` del directorio actual a `.docx`.

```bash
python doc2docx.py
```

Los archivos convertidos se guardan en `./output/`. Los archivos `.doc` originales no se modifican.
...
organizers/
├── requirements.txt
├── shared_utils.py          # shim de compatibilidad retroactiva
...
│   ├── fs.py                # busqueda y verificacion de acceso a archivos
│   ├── loaders.py           # registro de imports diferidos para dependencias opcionales
│   ├── log.py               # configuracion de logging
│   └── output.py            # ProgressReporter y helpers de escritura de archivos
├── readers/
│   ├── pages.py             # lectores puros de conteo de paginas (PDF, EPUB, CBZ, CBR, DOCX)
│   └── video.py             # lector puro de duracion de video
├── analyzers/
...
│   └── video.py             # analyze_flat() y analyze_series()
└── converters/
...
`shared_utils.py` es un shim de compatibilidad retroactiva que re-exporta simbolos de `core/` y `readers/`. El codigo nuevo debe importar directamente desde esos paquetes.
...
Todos los scripts usan el logger `organizers`. El nivel por defecto es `INFO`. `length.py` ademas escribe en `video_analyzer.log`. Usar `-l DEBUG` en `length.py` para salida detallada; los demas scripts solo tienen nivel INFO.
...
No se incluye archivo de licencia actualmente. Agregar un archivo `LICENSE` para definir los terminos de uso y distribucion.
